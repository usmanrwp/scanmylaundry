...
export default class index extends Component {
  ...
  componentDidMount = async () => {
    ...
    // const orderItemData = await cartDB.cartSelect();
  ...
  place_oder = async () => {
    ...
    if (this.state.totalPricee < 15) {
      Toast_("Our minimum order value is £ 15. Your order is below our minimum order value.");
      return;
    }
    ...
    if (this.state.PromoCode === '') {
      ...
      if (email === "empty") {
        ...
        if (res === 0) {
          ...
          let Tprice = this.state.totalPricee
        ...
        <Modal
          ...
          <TouchableOpacity
            ...
            <View style={{ width: "100%", marginTop:50 }}>
              <Text style={{color: '#000000', marginHorizontal: 22, marginTop: 3,
  ...
  next_btn = async () => {
    if (this.state.collectionDisplayTime === "Select Time") {
      if (Platform.OS === 'ios') {
        ...
        ToastAndroid.showWithGravity("Select Collection Time",ToastAndroid.SHORT, ToastAndroid.CENTER);
      }
    }
    else {
 ...
 */}
  ...
  creatingRecord = async () => {
    ...
    param["Total_Cart_All"] = this.state.totalCart;
    ...
    if (dis === 0) {
      param["total_bill"] = this.state.totalPricee;
    }
    else {
    ...
    let res = await _fetch2Param("place_order", param1, param);
    // alert(res);
    ...
    if (res.toString().split(':')[0] === "Record created successfully") {
      ...
      // Toast_("Order Completed Sucessfully");
      // return;
      ...
      let dis = this.state.DiscountedPrice;
      if (dis === 0) {
        payamount =this.state.totalPricee;
      }
      else {
