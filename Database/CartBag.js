    ...
    return new Promise(resolve => {
      db.initDB()
        .then(async db => {
          ...
          let len = 0;
          let totalPrice = 0;
          const st = "SELECT * FROM CartBag where Item_Id = " + Item_Id;
          ...
          if (len > 0) {
            db.transaction(async tx => {
              await tx
                .executeSql(
                  "UPDATE CartBag SET Quantity = ?, Total_Price = ? WHERE Item_Id = ?",
            ...
            db.transaction(async tx => {
              await tx
                .executeSql(
                  "INSERT INTO CartBag(Item_Id,C_ID, Name, Price,Quantity,Total_Price)  VALUES (?, ?, ?, ?,?,?)",
          ...
          const allPriceTotalQuery =
            "SELECT SUM(Total_Price) AS Total_Price from CartBag";
          await db.transaction(tx => {
    ...
    return new Promise(resolve => {
      db.initDB()
        .then(async db => {
          ...
          if (totalQuantity === 0) {
            db.transaction(async tx => {
              const delete_query = "DELETE FROM CartBag where Item_Id=" + Item_Id;
  ...
  cartItemTotalPrice = Item_Id => {
    return new Promise(resolve => {
      db.initDB()
        .then(db => {
          db.transaction(tx => {
            tx.executeSql("SELECT * FROM CartBag WHERE Item_Id = ?", [
              Item_Id
            ]).then(([tx, results]) => {
              if (results.rows.length > 0) {
                let row = results.rows.item(0);
    ...
    return new Promise(resolve => {
      ...
      db.initDB()
        .then(db => {
          db.transaction(tx => {
            tx.executeSql(
              "SELECT c.Name,c.ID, c.Item_Id,c.C_ID,c.Name,c.Price,c.Quantity,c.Total_Price,a.Picture,a.Item_Id as SC_ID FROM CartBag as c INNER JOIN Sub_Categories as a ON c.Item_Id=a.Item_Id"
            ).then(([tx, results]) => {
              var len = results.rows.length;
              for (let i = 0; i < len; i++) {
                let row = results.rows.item(i);
  ...
  clearCart = () => {
    return new Promise(resolve => {
      db.initDB()
        .then(db => {
          db.transaction(tx => {
            tx.executeSql("DELETE FROM CartBag")
              .then(([tx, results]) => {
  ...
  totalCartItems = () => {
    return new Promise(resolve => {
      db.initDB()
        .then(db => {
          db.transaction(tx => {
            tx.executeSql("SELECT SUM(Quantity) AS Quantity from CartBag").then(
              ([tx, results]) => {
